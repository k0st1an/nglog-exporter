...
func init() {
	...
	flag.StringVar(&config.Conf.Parse.LogFormat, "log-format", "_$host $request_method $status $request_length $request_time $body_bytes_sent $bytes_sent $upstream_status $upstream_connect_time $upstream_response_time $upstream_header_time $upstream_response_length", "log_format from nginx")
